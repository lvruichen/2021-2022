...
namespace SRobot
{
	...
	***********************************************************************/
	...
	void robotBackward(const double* TransVector, bool mconfig, double* theta)
	{
		...
		Matrix4d g1;
		Vector3d v1(0, 0, 0);
		g1 = R6toSE3(v1, w, th1[0]);
		cout << "gd: " << endl << gd << endl;
		t1 = inverseSE3(g3) * inverseSE3(g2) * inverseSE3(g1) * gd * inverseSE3(g0) * q;
		double theta4;
		subproblem1(p.head(3), q.head(3), t1.head(3), -1 * w, theta4);
